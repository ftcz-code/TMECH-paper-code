...
function [RRTree,pathFound,extendFail] = rrt_connect_bend(Checkpath,RRTreeA,RRTreeB,qgoal,stepsize,zstepsize,maxFailedAttempts,disTh,map)
pathFound=[]; 
failedAttempts=0;
Index = RRTreeA(1,4);
while failedAttempts <= maxFailedAttempts
    if Index == 0 %%********************************扩展RRTree1的方式************************************
        ...
        if distanceCost2(RRTreeB(I2(1),1:3),newPoint) < disTh         
            %% 检测两棵树相连的线段之间是否存在障碍物
            if  Checkpath(RRTreeB(I2(1),1:3), newPoint, map)==1
        ...
        while flag == 1
           ...
           %% 检测新节点与另一个树的节点是否满足阈值
            ...
            if distanceCost2(RRTreeB(I2,1:3),newPoint) < disTh  
                %% 检测两棵树相连的线段之间是否存在障碍物
                if  Checkpath(RRTreeB(I2,1:3), newPoint, map)==1
                    pathFound= [newPoint I I2];
                    extendFail = 0;
