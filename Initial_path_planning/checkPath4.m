...
function feasible=checkPath4(n,newPos,map)
feasible=1;
newPos1=newPos;
newPos2=newPos;
newPos3=newPos;
...
% 扩展点碰撞检测
for r=0:0.5:sqrt(sum((n(1:2)-newPos(1:2)).^2))
    posCheck=n(1:2)+r.*[sin(dir) cos(dir)];
    if ~(feasiblePoint(ceil(posCheck),map) && feasiblePoint(floor(posCheck),map) && ... 
            feasiblePoint([ceil(posCheck(1)) floor(posCheck(2))],map) &&...
            feasiblePoint([floor(posCheck(1)) ceil(posCheck(2))],map))
        feasible=0;break;
    end        
     if ~feasiblePoint_bend(newPos,map), feasible=0; end %判断机器人中心点是否在图中且与障碍物是否碰撞
end
